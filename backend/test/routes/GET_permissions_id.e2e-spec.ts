...
describe('Rota GET "/permissions/:id', () => {

    let app: INestApplication<App>;

    let body = {
        email: 'test_get_permissions_id@test.com',
        password: 'test_get_permissions_id'
    }; 
    ...
    it('Tenta acessar a permissão sem o devido acesso.', async () => {
        await checarTokenAuth(app, '/permissions/'+permission.id, 'get', access_token);
    });

    it('Tenta acessar a permissão com as permissões corretas.', async () => {
        user.roleId = 1;
        await user.save();

        access_token = jwtService.sign({ sub: user.id, email: user.toJSON().email, roleId: 1 });
        const response = await sendRequest(app, 'get', '/permissions/'+permission.id, {}, access_token);
